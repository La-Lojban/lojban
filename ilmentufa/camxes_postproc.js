...
function camxes_postprocessing(input, mode) {
    ...
    if (!mode) mode = "MTC";
		if (is_string(mode)) {
        const with_spaces       = among('S', mode);
        const with_morphology   = among('M', mode);
        const with_terminators  = among('T', mode);
        const with_trimming     = !among('R', mode);
        const with_selmaho      = among('C', mode);
        const with_nodes_labels = among('N', mode);
        const with_json_format  = among('J', mode);
				/* Calling the postprocessor */
				let output = newer_postprocessor(input, with_morphology, with_spaces,
...
function newer_postprocessor(
    ...
    /* Building a map of node names to node value replacements */
		const value_substitution_map = with_spaces ? {"spaces": "_", "initial_spaces": "_"} : {};
    ...
    /** Building a node_action_for() function from the selected options **/
		const is_flattening_target = with_morphology ? 
					function (tree) { return false; } 
					: function (tree) {
							const targets = special_selmaho;
							return (among(tree[0], targets) || is_selmaho(tree[0]));
						};
    const is_branch_removal_target = function (tree) {
