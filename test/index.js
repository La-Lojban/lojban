...
describe("#romoi_lahi_cmaxes", function () {
  it('parses \'coi ro do\' as \'[["cmavo","coi"],["cmavo","ro"],["cmavo","do"]]\'', function () {
    JSON.stringify(
      lojban.romoi_lahi_cmaxes("coi ro do")["te spuda"]
    ).should.equal(
      '[["cmavo","coi"],["cmavo","ro"],["cmavo","do"]]'
    )
  })
})
...
describe("#jvokaha_gui", function () {
  it("should split 'xonka'a' into ['-xon-','katna']", function () {
    JSON.stringify(lojban.jvokaha_gui("xonka'a")).should.equal(
      '["-xon-","katna"]'
    )
  })
})
