...
import { is_permissible } from "./morphology"
const exp_rafsi = true

function create_every_possibility(aa: any[]) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    result = result.concat(
      create_every_possibility(arr_arr).map((f: string[]) => {
        return f.concat([e])
      })
    )
  }
...
function gismu_rafsi_list$(a: string) {
  if (typeof gismu_rafsi_list[a] !== "undefined") {
...
function get_candid(selrafsi: string, isLast: boolean) {
...
export function search_selrafsi_from_rafsi2(rafsi: string): string | null {
...
function get_lujvo_score(rafsi_ynr_sequence: Array<string>) {
...
export function jvokaha(lujvo: string) {
...
export function jvokaha2(lujvo: string): string[] {
  ...
  while (true) {
    ...
    if (
      get_CV_info(lujvo.slice(0, 3)) === "CVV" &&
    ...
    if (
      get_CV_info(lujvo.slice(0, 5)) === "CVCCY" ||
    ...
    if (
      get_CV_info(lujvo.slice(0, 3)) === "CVC" ||
...
export function jvozba(arr: string | any[], forbid_la_lai_doi?: boolean) {
  let candid_arr = []
  ...
  const answers = create_every_possibility(candid_arr)
    ...
    .sort(function (a: { score: number }, b: { score: number }) {
...
function is_cmevla(valsi: string) {
  return (
    valsi.length >= 1 &&
...
function normalize(rafsi_list: any[]) {
...
function is_tosmabru(rafsi: any, rest: string | any[]) {
  //skip if cmevla
  ...
  for (var i = 0; i < rest.length; i++) {
    ...
    if (
      ...
      (get_CV_info(rest[i]) === "CVCCV" &&
        2 === is_permissible(rest[i].charAt(2), rest[i].charAt(3)))
    ) {
      break
      // further testing
    } else {
