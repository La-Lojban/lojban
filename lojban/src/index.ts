...
import {
	zeizei as zeizei_local,
	rotpaci as rotpaci_local,
} from "./ceha/zeizei";
import { anji as anji_local } from "./ceha/anji";
import { modzi as modzi_local } from "./ceha/modzi";
import { lojban2ipa as lojban2ipa_local } from "./ceha/ipa";
...
export function loglytufa_master(te_gerna: any) {
	try {
		const terspuda = require("./libs/loglan_postproc").postprocessing(
			require("../libs/loglytufa/loglan.js").parse(te_gerna, {
...
export function ilmentufa_off(te_gerna: any, mode: any, preprocess: any) {
	if (preprocess) te_gerna = preprocessing(te_gerna);
	try {
		const terspuda =
			require("../libs/ilmentufa/camxes_postproc.js").postprocessing(
...
export const lojban2loglan = (te_gerna: string) => {
	return require("./bangu/loglan").lojban2loglan(te_gerna, romoi_lahi_cmaxes);
};
